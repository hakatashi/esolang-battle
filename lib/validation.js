...
module.exports.validate = async ({submission, language, solution, contest, noInputGeneration = false}) => {
	...
	const traceFlag = !["bash-busybox", "bash-pure"].includes(language.slug);
		...
		if (populatedSubmission.status === 'success' && contest.id !== 'hello') {
			const bytesInfo = (() => {
				if (solution) {
					return `${
			...
			try {
				discord.send(`**${populatedSubmission.user.name()}** won the language **${
					language.name
				}**!! (${bytesInfo}) Congrats!!!\nhttps://eso.hideo54.com/submissions/${
