...
module.exports.validate = async ({
	...
	try {
		...
		const infos = await Promise.all([
			...
			id: language.slug,
			code: newSubmission.code,
			stdin: submissionInput[1],
			trace: false, // TODO true?
			disasm: false,
                   })]);
		console.log('info:', infos);

		if (typeof infos[0] !== 'object' || typeof infos[1] !== 'object') {
			throw new Error('info is not object');
		}

                for (let i = 0;i<2;i++) {
                   const {stdout, stderr, duration, error, trace} = infos[i];
