	...
	api.get('/languages').then((languages) => {
		...
		$('.language').each((index, languageEl) => {
			...
			if (language) {
				$language.attr(
					'data-toggle',
					competitionEnd || language.available ? 'modal' : false
				);
		...
		$('.team-info.red > .bar').css({
			flexBasis: `${red / (red + blue + green) * 100}%`,
		});

		$('.team-info.blue > .score').text(blue);
		$('.team-info.blue > .bar').css({
			flexBasis: `${blue / (red + blue + green) * 100}%`,
		});

		$('.team-info.green > .score').text(green);
		$('.team-info.green > .bar').css({
			flexBasis: `${green / (red + blue + green) * 100}%`,
		});
	});
};
...
$(document).ready(() => {
	...
	$('.language').each((index, languageElement) => {
		...
		$language.click(() => {
			...
			if (language.solved) {
				const solutionURL = `${location.origin}/submissions/${
					language.solution._id
				}`;
				$modal
					.find('.owner-name')
					.text(
						`${language.solution.user} (${
							['Red', 'Blue', 'Green'][language.team]
						})`
					);
				$modal
					.find('.solution')
					.text(language.solution._id)
					.attr('href', solutionURL);
				...
				api.get('/submission', {_id: data._id}).then((submission) => {
					...
					if (submission.status === 'failed') {
						$modal
							...
							.addClass('bg-warning')
							.html(
								`<strong>Submission failed.</strong> Check out the detail <a href="${
									location.origin
								}/submissions/${submission._id}" target="_blank">here</a>.`
							)
							.show();
						$modal.find('.submit-code').attr('disabled', false);
						...
						$modal
							...
							.addClass('bg-success')
							.html(
								`<strong>You won the language!</strong> Check out the detail <a href="${
									location.origin
								}/submissions/${submission._id}" target="_blank">here</a>.`
							)
							.show();
